...
def resume_env(plot=False,  # To plot results (Field, controls, lift, drag, rec area) during training
    ...
    # Warning: Only choose one of the following parameters. These control the type of state input to NN
    single_input = True
    asym_input = False
    antisym_input = False
    force_input = False
    ...
    root = '/home/issay/UROP/DPC_Flow/Environment/mesh/turek_2d'  # Root of geometry file path
    if(not os.path.exists('/home/issay/UROP/DPC_Flow/Environment/mesh')):
        os.mkdir('/home/issay/UROP/DPC_Flow/Environment/mesh')
    ...
    output_params = {'locations': list_position_probes,  # List of (x,y) np arrays with probe positions
                     ...
                     'symmetric':False,
                     'asym_input':asym_input,
                     'antisym_input':antisym_input,
    ...
    optimization_params = {"num_steps_in_pressure_history": 1,  # Number of steps that constitute an environment state (state shape = this * len(locations))
                        ...
                        "smooth_control": False,  # parameter alpha to smooth out control
                        "no_control": True, # Disable control if true. Have to set "smooth_control to False"
    ...
    if(remesh):
        ...
        if (os.path.exists('/home/issay/UROP/DPC_Flow/Environment/mesh')):
            shutil.rmtree('/home/issay/UROP/DPC_Flow/Environment/mesh')  # If previous mesh directory exists, we delete it
        os.mkdir('/home/issay/UROP/DPC_Flow/Environment/mesh')  # Create new empty mesh directory
