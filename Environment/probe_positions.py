...
def probe_positions(probe_distribution, geometry_params):
    ...
    'rabault9' - Partial information distribution tested by Rabault in the appendix E of his first 2019 paper.
    'inflow8' & 'inflow64' - Distribution used in Lucien Viala's project thesis 2020
    ...
    if (distribution_type == 'rabault151' or distribution_type == 'rabault241'):
        ...
        elif distribution_type == 'around':

            positions_probes_for_grid_x = length_cylinder / 2
            positions_probes_for_grid_y = [-height_cylinder / 2 + (height_cylinder / (n_base + 1)) * i for i in range(1, n_base + 1)]
            n_top_and_bottom = 8
            positions_probes_for_top_and_bottom = [-length_cylinder / 2 + (length_cylinder / (n_top_and_bottom + 1)) * i for i in range(1, n_top_and_bottom + 1)]
            positions_probes_for_top_y = height_cylinder / 2
            positions_probes_for_bottom_y = -height_cylinder / 2

            for crrt_y in positions_probes_for_grid_y:
                list_position_probes.append(np.array([positions_probes_for_grid_x, crrt_y]))  # Append (x,y) pairs np array as probes at base

            for crrt_x in positions_probes_for_top_and_bottom:
                list_position_probes.append(np.array([crrt_x, positions_probes_for_top_y]))  # Append (x,y) pairs np array as probes at top and bottom
                list_position_probes.append(np.array([crrt_x, positions_probes_for_bottom_y]))
